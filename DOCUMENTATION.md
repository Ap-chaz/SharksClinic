...
5. **Around 40 sidebar sub-items exist** (Register Patient, Patient Search, Patient Profiles, Doctor Schedules, Appointment Status, Payments, Outstanding Balances, and many more unrelated sections like Lab, Pharmacy, Reports, Settings) **but only the four explicitly requested modules — Patients, Doctors, Appointments, Billing — were built as full views.** Other sidebar items show a "part of a future update" toast rather than a broken/dead click, which is more honest than pretending they work.
6. **The feedback form saves to a `feedback` collection in `SharksDB`,** but no admin view was built to display submitted feedback, since a "Feedback" admin page wasn't part of the original page set or this request's explicit scope. The data is captured and ready to display whenever that view is added.
